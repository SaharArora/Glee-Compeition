...
| `persuasion_calibration_bins` | Audit and prospectively recalibrate under-confidence in `P(high|recommend=yes)` without changing the evidence accessor. | Evaluate the single preregistered predictive candidate below; no policy edit or default flip is authorized by predictive evidence. | In 0.5-0.8, model holdout n=6,658 gap -0.1738 and config holdout n=4,858 gap -0.1345. Estimand mismatch is not the main cause: only 543/35,596 prior-purchase states contain an after-no buy; all-after-yes gap remains -0.1075. | `candidate` | `calibration_bins` | Shared `_persuasion_beliefs`/buy path with `persuasion_accessor`, `persuasion_explore`, and deceptive-seller work; instrumentation only so far. | `reports/persuasion_calibration/persuasion_calibration.json`; `glee_eval/diagnostics/persuasion.py`; `tests/test_persuasion_calibration.py` |
