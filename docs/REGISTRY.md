...
| `model_b_mixed_fold_crossfit` | Exhaustive three-fold actor-model OOF (5 of the 15 real identities per fold) plus four-fold canonical-config OOF, retaining hierarchical response estimates and joint-bundle validation. | Rerun the FIT-only actor-fold-0 kill-check with the bounded-memory Newton-PCG instrument before any validation. | Mixed manifest passed 5/5/5. Three numerical convergence failures and one memory failure are retained in FAILED. Newton-PCG and its outer validator passed dense/adversarial review. The exit-137 mechanism is repaired by shared fixed-state Neumaier accumulators: objective O(1), gradient/HVP O(parameters), raw KKT O(models+configs). Cancellation at `.999e-7`/`1.001e-7` matches `math.fsum`, and row order is deterministic. Current pre-holdout evidence: 82 Newton-PCG/core tests pass; 66 focused validator/solver/crossfit/opponent tests pass. The outer validator independently rejects the old coordinate optimizer and reconstructs final/inner raw-KKT, zero-sum/order-hash, PCG target/cap/shift/residual/curvature/descent, projected-Armijo, pooled-CV and selected-ridge contracts. No corpus-fit or predictive result is claimed here. | `candidate` | unassigned | Model/ridge/folds/raw-KKT/PCG/Armijo/endpoints remain frozen. Actor fold 0 remains the next FIT-only checkpoint; no holdout scoring, payoff gate or live play authorized. | Prospective mixed-fold and `model_b_response_newton_pcg` declarations below; `glee_eval/diagnostics/joint_population.py`; `tests/test_joint_population_validation.py`; `tests/test_hierarchical_opponent_responses.py`; `docs/FAILED.md` |
