...
| `model_b_mixed_fold_crossfit` | Exhaustive three-fold actor-model OOF (5 of the 15 real identities per fold) plus four-fold canonical-config OOF, retaining hierarchical response estimates and joint-bundle validation. | Repair the training optimizer without changing its penalized-logistic objective, then prove predictive value on every fold before any tournament or policy gate. | Mixed manifest passed 5/5/5. FIT-only actor fold 0 exposed an instrument failure before scoring: all B/N/P response estimators hit 300 iterations with `converged=false`. The run stopped during fold 1; no holdout was scored. Exact objective-equivalent solver repair is logged in FAILED. | `candidate` | unassigned | Per-axis manifest/router, hierarchical response fit, joint validator. No payoff gate or live play authorized. | Prospective mixed-fold declaration below; `docs/FAILED.md` |
