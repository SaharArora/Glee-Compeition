...
| `guarantee_own_margin` | Optional own-profitability offer clip and live `counter_price` plumbing (`guarantee_own_margin=False`). | Independent confirmation declared below; default remains false until it clears the identical predicate and every conditional/ordinary check. | Seed 4242, n=1600: ordinary +0.0076045, t=9.456, 117W/0L; conditional n=1068, +0.0102835, t=8.770, 96W/0L, zero regressing archetype/config subgroups. | `gate-passed` | `construction_defect_gate` | Shared negotiation counteroffer path; smallest fix changes only the collapsed clip bound and coupled counter-price exposure. | `/private/tmp/glee-guarantee-margin-conditional/verdict.json`; `docs/PROMOTION_CRITERIA.md` |
...
| `live_schema_history` | Re-verify live fixtures and translation assumptions against current official glee-sdk 0.0.5 documentation. | Only partially verified: all fixtures omit documented `game_state.history`, persuasion fixtures omit `current_player`, and the adapter ignores documented history. | Action vocabularies/scalars match docs. No local `glee_sdk`, so integration tests skip. No live connection made. | `candidate` | unassigned | Highest pre-live integration risk; never authorizes a live/rated game. | `docs/LIVE_INTEGRATION.md`; official glee-sdk docs/source |
