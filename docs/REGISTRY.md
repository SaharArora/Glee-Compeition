...
| `h6_percentile` | Decide whether shadow negotiation percentiles should stratify gains/no-trade regimes. | Human decision: better skill measure versus unknown alignment with official pooled scoring. | Diagnostic confirmed: no-trade 0.385 pooled vs 0.508 stratified; gains 0.769 pooled vs 0.599 stratified. Reported, not corrected. | `confirmed` | unassigned | Scoring/reporting only; does not touch agent policy. | `docs/HANDOVER.md` §2/§6 |

## Session kill-checks

Record the smallest real-log contradiction check here before a new approach family begins.
No new family has begun in this seeding change.
