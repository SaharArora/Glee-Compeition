...
| `live_strict_game_limit` | Replace reliance on SDK `max_games` with bounded matchmaking waves and a unique-game-ID cap including opponent-ended games. | Upstream `--max-games 12` produced 31 terminals; repository wrapper no longer delegates this limit. | Real verification completed exactly 75/75, 25 per family; 36 direct +39 backfill, zero errors/fallbacks/schema violations. | `shipped` | none | Do not call upstream `GleeClient.run(max_games=...)` directly. No further live games without explicit authorization. | `docs/FAILED.md`; `tests/test_live_run.py` |
| `live_simulator_alignment` | Diagnose live/offline payoff differences against fitted opponent/config assumptions without changing policy. | Large confirmation gaps did not replicate at volume; causal policy-regression premise rejected. | Volume B .441103, P .382000 (n=25 each); live persuasion buyer purchase 61/260=23.5% vs offline 49.94%. Fitted persuasion opponents omit language content and sampled memory behavior. | `candidate` | unassigned | Next work is observed-config replay and language/history-capable opponent measurement; normal gate required before any policy change. | `docs/FAILED.md`; `reports/live/volume_20260815` |
| `persuasion_text_stance` | Default-off buyer parser for unequivocal natural-language recommendations when the live text payload contains no structured yes/no stance; ambiguous text remains a conservative decline and binary inputs are unchanged. | Production mechanism diagnosed: 180/180 text buyer turns across nine complete live games defaulted to no despite 101 clearly positive and 79 clearly negative messages. The current simulator masks the defect by injecting `buy_no_buy` beside text. | **Prospective gate declaration:** frozen 420-turn replay must classify all 180 directional text turns with zero polarity errors, preserve all 240 binary translations and raw messages, and pass adversarial negation cases. Payoff gate: seed 314159, n=1600, persuasion only, model-held-out opponent population plus config-held-out catalogue, with a live-contract simulation path that exposes text without the latent structured stance. Every ordinary promotion check is mandatory. | `candidate` | root | Shares persuasion transcript parsing with the live adapter, synthetic runner, fitted opponent policy, and buyer decision path. No live payoff claim is allowed from declined rounds because their qualities are unobserved. | `reports/live/confirmation_20260815`; `reports/live/volume_20260815`; `docs/FAILED.md` |
