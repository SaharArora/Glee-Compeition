...
| `live_persuasion_value_visibility` | Require `u/v` only when visible: buyer turns and informed-seller turns; preserve legitimate absence for `is_seller_know_cv=false` seller turns. | Fixed and replay-verified; further live volume remains paused until the user decides the offline replay is sufficient or separately authorizes re-verification. | Raw log: 120 turns = 6 uninformed-seller games x 20 rounds; no alternate names existed. Corrected contract replays all 1,423 captured payloads with zero violations; 348 tests pass, 7 skip. | `shipped` | none | Validation only; translation/action policy unchanged. | `reports/live/observations.jsonl`; `docs/FAILED.md`; `tests/test_contracts.py` |
