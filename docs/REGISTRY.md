...
| `deceptive_seller_guard` | Measure whether a past-only sparse-dishonesty signal justifies a sample-size-aware lower bound on `P(high|recommend=yes)`; no acting guard yet. | Define exactly one default-off acting guard and a prospective payoff gate, or leave diagnostic-only. Myopic rows remain out of scope because the evidence is unobservable. | Matched diagnostic survives both holdouts after exact p/v/c, round, memory, message, and evidence-count stratification. Model: overconfidence contrast +0.11754, surplus contrast -0.60565, effective reach 2,661/13,725 (19.39%), 283 games. Config: +0.10194, -0.56272, reach 1,726/9,153 (18.86%), 213 games. | `candidate` | unassigned | Persistent persuasion buyer posterior only; shared surface with rejected Platt map and exploration. One-step evidence is not trajectory payoff evidence. | `reports/persuasion_past_dishonesty/persuasion_past_dishonesty.json`; `glee_eval/diagnostics/persuasion_dishonesty.py`; `tests/test_persuasion_dishonesty.py` |
