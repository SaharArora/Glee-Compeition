...
- `model_b_mixed_fold_crossfit` prospective correction (declared 2026-08-15 after the
  ...
  **20260815**; 256 draws; 2,000 clustered bootstraps; both energy and CRPS requirements; every
  ...
  in each of four config folds. These 12/20 overall and >=3-per-fold requirements apply to every
  mandatory decision-channel cell as well as every bundle family/axis cell, and explicitly
  supersede the retracted four-by-four declaration's `>=5 split clusters` decision-cell floor.
...
- `model_b_response_audit_normalization` FIT-instrument declaration (declared 2026-08-16 after
  ...
  mixed outer folds, three inner game-hash folds, ridge grid and pooled-loss selection, zero-sum
  Newton-PCG solver/constants, original-coordinate KKT contract, bundle sampler, comparators,
  predictive endpoints and every existing threshold. This declaration changes only the logical
  ownership, representation and lifetime of already-required solver evidence.

  Store one canonical complete final/inner-CV response audit per family in the main population
  artifact. Do not copy its nested KKT/objective, active-bound, PCG shift/residual/curvature/
  descent or Armijo histories into every bundle parameter. Each compact bundle response entry
  must instead contain its family, response channel, an immutable canonical-fit reference, and
  only parameter-specific kind/value/threshold metadata plus channel support. The reference must
  resolve uniquely within the same artifact. If implementation instead requires an external
  audit object, its format and immutable byte SHA must be declared prospectively before use; no
  untracked sidecar is authorized by this declaration.

  The validator must verify reference completeness and uniqueness, reject missing, orphaned,
  cross-family/channel or tampered references, reconstruct every bundle response value from the
  referenced coefficients and `x_scale`, and prove exact prediction/evidence equivalence to an
  unnormalized fixture. It must continue independently checking every previously declared final
  and inner-CV zero-sum/order-hash, raw-KKT, objective, active-slope, PCG, projected-Armijo,
  finite-loss, pooled-CV and selected-ridge condition. Hashes or summaries may deduplicate an
  audit but may not replace numerical records needed for those checks.

  Write the artifact atomically with streaming JSON so neither a second whole object tree nor a
  complete serialized string/bytes copy is resident in memory. Release phase-local event rows,
  response rows, sufficient statistics and transient fit state only after their last consumer;
  release may not occur early enough to change fitting, bundle construction, provenance or
  validation. Serialize explicit schema version, canonical-fit identifiers and hashes, audit and
  bundle counts, reference counts, atomic-write method, and phase-local memory/release provenance.
  Before the next corpus checkpoint require focused tests for normalized-versus-legacy logical
  and prediction equality, reference resolution and tamper rejection, all-family/channel support,
  deterministic hashes/bytes, atomic failure leaving no partial artifact, no whole-tree/string
  copy in the writer, and bounded peak memory under bundle-count scaling. Actor fold 0 remains the
  ...
  **Repaired storage result under the later materially-new Decimal-Armijo/canonical-attachment
  formulation: successful as a FIT instrument.** All seven frozen artifacts pass compact-reference
  completeness, presence, hash, reconstructed-value and router audits with zero fit/reference
  errors. This does not revive the rejected `a6ac5e3b...` artifact and is not predictive evidence.
...
- `model_b_response_decimal_armijo` materially-new FIT-instrument declaration (declared
  ...
  **FIT result: successful; predictive status untested.** At code checkpoint `55abea0`, 79 combined
  local tests and 69 focused independent tests passed. The frozen manifest logical SHA256 is
  `44fe77cb413a182b741b81115876bd9eeae15a4b83d80126ae9542fab03c0fc2`.
  Actor artifact SHA256 values by fold 0/1/2 are
  `d8767e442e3c20d2d554ecdf8847e211f53c7160109f6207be8be38723072239`,
  `5c78e0182c0276b3b226e394da11895257bd45ec37eab070c5ce455a48842875`, and
  `0ba5468e5d8a66bea863535be3cb45e9e1b9a8babf7684d6ef179ac77fe644e2`.
  Config artifact SHA256 values by fold 0/1/2/3 are
  `7eb4e104af7fe70397aa1f76b2632a1630b29db623087b0e180fa5c90ffe231f`,
  `d47763694db27c83c7549b82e5fae7f76caacfd5657b16e6e229809397be9cd1`,
  `e32dfe036267e6fc7fdddcda35caa6300c0cdd33977b36e1e05fd15526f56aae`, and
  `f3ec0b1b136654a319a2fa218e43489113aa7855e45a59cb00494a9cddb4bea2`.
  All 21 family fits are finite and `status=ok`; the worst final/inner original-coordinate KKT is
  `9.990772298351658e-08`; fit and compact-reference audits have zero errors; and both-axis router
  checks pass. No holdout was extracted or scored while freezing these artifacts. This result
  authorizes only the unchanged, already-declared mixed-fold holdout validation and does not
  establish predictive success or authorize payoff/default/live work.
