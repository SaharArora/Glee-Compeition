...
| `persuasion_calibration_bins` | Optional default-off Platt map of `P(high|recommend=yes)` using model-FIT parameters `a=0.3651090145`, `b=1.1369808568`; apply only to the buy decision after a yes recommendation and retain raw posterior diagnostics. | Implement mechanics/tests, then run one paired n=1600 structural-holdout promotion evaluation at seed 271828. If it passes, declare a different confirmation seed/sample before running confirmation. | Predictive prerequisite passed both axes. Model holdout Brier delta -0.01044, 95% clustered CI [-0.01222,-0.00873], log loss -0.03179 [-0.03637,-0.02715]. Config holdout Brier -0.00858 [-0.01148,-0.00586], log loss -0.02458 [-0.03242,-0.01694]. Closest FAILED entry is frozen-posterior severity retraction; materially different because this candidate is fit-only calibration with no accessor/severity claim. | `candidate` | `calibration_bins` | Shared `_persuasion_beliefs`/buy path with `persuasion_accessor`, `persuasion_explore`, and deceptive-seller work. Default must remain false unless confirmed. | `reports/persuasion_calibration/persuasion_calibration.json`; `glee_eval/diagnostics/persuasion.py`; `tests/test_persuasion_calibration.py` |
