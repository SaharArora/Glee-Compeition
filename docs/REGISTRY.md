...
| `model_b_mixed_fold_crossfit` | Exhaustive three-fold actor-model OOF (5 of the 15 real identities per fold) plus four-fold canonical-config OOF, retaining hierarchical response estimates and joint-bundle validation. | Repair the Newton-PCG accumulator's memory complexity without changing its frozen math, then rerun the FIT-only actor-fold-0 kill-check before any validation. | Mixed manifest passed 5/5/5. Three numerical convergence failures and the latest boundedness failure are retained in FAILED. Newton-PCG passed dense/adversarial review and its outer provenance validator, but the first corpus fold was killed with exit 137 before an artifact write. The pre-run actor-fold-0 file retained its old 2026-08-15 timestamp, proving no new fit was accepted. Stable summation had materialized per-coordinate term lists inside repeated gradient/HVP/KKT passes; peak memory, not model stationarity, killed the process. | `candidate` | unassigned | Replace only list materialization with deterministic compensated streaming sums and demonstrate numeric equivalence + bounded memory before rerun. Model/ridge/folds/KKT/PCG/Armijo/endpoints remain frozen. No holdout scoring, payoff gate or live play authorized. | Prospective mixed-fold and `model_b_response_newton_pcg` declarations below; `docs/FAILED.md` |
