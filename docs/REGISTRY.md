...
| `deceptive_seller_guard` | Measure whether a past-only sparse-dishonesty signal justifies a sample-size-aware lower bound on `P(high|recommend=yes)`; no acting guard yet. | Build diagnostic using production-visible history only. Kill unless both model and config holdouts show positive posterior overconfidence and worse value outcomes after prior lies, with >=200 changed decisions, >=30 games, and >=1% reach. | Synthetic one-lie probe still buys at posterior .5714/honesty .3333 for p=.5, v=2, c=0; hypothesis only. Closest FAILED entries are the frozen-posterior severity retraction and rejected global Platt map; materially different because this tests uncertainty conditional on past dishonesty rather than accessor/calibration alone. | `candidate` | `deceptive_seller` | Same persuasion posterior/buy surface as calibration and exploration; diagnostic only until premise and reach survive both holdouts. | `docs/PROMOTION_CRITERIA.md`; `glee_eval/diagnostics/persuasion.py`; `my_agents/jordan_strategic.py` |
