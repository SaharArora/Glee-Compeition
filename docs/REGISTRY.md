...
| `live_terminal_results` | Audit reconstructible outcomes with `live-episodes`, capture full SDK move responses, and GET-backfill games ending on opponent moves. | Existing observation log has no authoritative terminal result; the repair is prospective and awaits an authorized real verification batch. | Historical 109 games: bargaining 15 reconstructed/21 indeterminate; negotiation 13/23 (7 lack normalization); persuasion 19/18. Direct and backfill capture paths are fixture-tested. | `candidate` | unassigned | Next 20-30-game verification requires fresh explicit user authorization; require near-100% terminal coverage or return to diagnosis. | `docs/LIVE_INTEGRATION.md`; `glee_eval/live/episodes.py`; `glee_eval/live/run.py` |
