...
| `model_b_mixed_fold_crossfit` | Exhaustive three-fold actor-model OOF (5 of the 15 real identities per fold) plus four-fold canonical-config OOF, retaining hierarchical response estimates and joint-bundle validation. | Run the FIT-only actor-fold-0 kill-check with the preregistered Newton-PCG instrument, then prove predictive value on every fold before any tournament or policy gate. | Mixed manifest passed 5/5/5. Three failed optimizers are retained in FAILED. The replacement uses exact zero-sum contrasts plus full sparse Hessian-vector Newton-PCG while preserving the statistical model and raw KKT `1e-7` contract. Dense HVP/PCG, arbitrary zero-sum, bound-slope, failure-path, multi-channel and production/shared-primitive checks pass; independent adversarial review is GO and 50 integrated tests pass. No corpus fold has yet been accepted under this solver. | `candidate` | unassigned | Per-axis manifest/router, hierarchical response fit, joint validator. Outer provenance enforcement is being updated before actor fold 0; no holdout scoring, payoff gate or live play authorized. | Prospective mixed-fold and `model_b_response_newton_pcg` declarations below; `docs/FAILED.md` |
