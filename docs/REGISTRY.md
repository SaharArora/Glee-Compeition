...
| `live_strict_game_limit` | Replace reliance on SDK `max_games` with a provable total-game cap including opponent-ended games. | `--max-games 12` produced 31 distinct terminal games because SDK accounting counts only games ending on our move and concurrency overshoots. | Real confirmation batch; no synthetic inference. | `candidate` | unassigned | Must be fixed before another authorized run; lowering the SDK number is not sufficient. | `docs/FAILED.md`; `reports/live/confirmation_20260815/run_summary.json` |
