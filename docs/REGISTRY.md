...
| `deceptive_seller_guard` | Default-off persistent-buyer one-standard-deviation lower bound after a production-visible prior lie. | Gate rejected decisively; do not rerun or tune this bound. Any future direction must explain why one-step matched evidence reversed under trajectory simulation. | Seed 161803, n=1600 structural holdout: -0.0052, t=-5.40, 9W/61L/1530T. Failed effect, significance, archetype breadth 0.9375, and config breadth 0.5000. All persistent regimes lost; myopic regimes were unchanged. | `candidate` | unassigned | Shared persuasion buy path. Default remains false; no confirmation declared. Observational audit remains a diagnostic association, not evidence that blocking purchases improves trajectories. | `reports/promotion/deceptive_seller_guard_seed161803/promotion_verdict.json`; `docs/FAILED.md`; `tests/test_persuasion_mechanics.py` |
