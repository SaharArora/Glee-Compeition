...
| `model_b_mixed_fold_crossfit` | Exhaustive three-fold actor-model OOF (5 of the 15 real identities per fold) plus four-fold canonical-config OOF, retaining hierarchical response estimates and joint-bundle validation. | Complete the frozen corpus fits, then prove predictive value on every fold before any tournament or policy gate. | Mixed manifest passed 5/5/5. Two failed optimizers are retained in FAILED. The current exact-objective sparse coordinate-Newton repair requires projected-KKT `<=1e-7`, rejects stagnation/nonconverged inner folds, and records every ridge/fold loss and stop state. The outer validator independently reconstructs pooled CV selection and rejects invalid provenance before scoring. Integrated 66-test suite and a synthetic end-to-end fit pass with zero provenance errors; full-corpus fold convergence remains unmeasured. | `candidate` | unassigned | Per-axis manifest/router, hierarchical response fit, joint validator. Actor fold 0 is the next FIT-only kill-check; no holdout scoring until all artifacts freeze. No payoff gate or live play authorized. | Prospective mixed-fold declaration below; `docs/FAILED.md` |
