...
| `live_schema_history` | Align fixtures/contracts/adapter with current official glee-sdk 0.0.5 `history` and persuasion `current_player` schema. | Docs-level repair complete; real-server payload compatibility remains unverified until a separately authorized game. | All seven fixtures now carry history; adapter consumes all documented family histories with last-offer/totals compatibility fallbacks. Full suite 340 passed, 7 skipped; local SDK integration remains skipped because `glee_sdk` is not installed. | `candidate` | unassigned | Highest remaining pre-live risk is server verification; never authorizes a live/rated game. | `docs/LIVE_INTEGRATION.md`; official glee-sdk docs/source; `tests/test_live_adapter.py` |
