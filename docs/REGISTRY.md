...
| `model_b_mixed_fold_crossfit` | Exhaustive three-fold actor-model OOF (5 of the 15 real identities per fold) plus four-fold canonical-config OOF, retaining hierarchical response estimates and joint-bundle validation. | Repair the training optimizer without changing its penalized-logistic objective, then prove predictive value on every fold before any tournament or policy gate. | Mixed manifest passed 5/5/5. The original globally scaled gradient stalled. Its diagonal-Newton replacement was killed before a fold completed: 10/10 ordinary mixed actor/config fixtures missed the fixed tolerance, and tiny backtracked steps could falsely set `converged=true` while the projected gradient remained large. The corpus rerun was interrupted during actor fold 0; no new artifact or holdout score was accepted. | `candidate` | unassigned | Per-axis manifest/router, hierarchical response fit, joint validator. Any next solver must preserve the declared objective/folds/tolerance, enforce projected KKT stationarity, and reject nonconverged inner-CV fits. No payoff gate or live play authorized. | Prospective mixed-fold declaration below; `docs/FAILED.md` |
