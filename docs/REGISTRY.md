...
| `model_b_mixed_fold_crossfit` | Exhaustive three-fold actor-model OOF (5 of the 15 real identities per fold) plus four-fold canonical-config OOF, retaining hierarchical response estimates and joint-bundle validation. | Prove predictive value on every actor/config fold before any tournament or policy gate. | Closest failure is the impossible four-by-four declaration. Material difference: the manifest and artifact router now model the actual finite actor population with per-axis fold counts/fractions; every one of 15 identities is evaluated exactly once with 10 training identities. Exact prospective correction below. | `candidate` | unassigned | Per-axis manifest/router, hierarchical response fit, joint validator. No payoff gate or live play authorized. | Prospective mixed-fold declaration below; `docs/FAILED.md` |
