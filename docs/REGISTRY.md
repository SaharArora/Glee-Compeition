...
| `uninformed_seller_coverage_key` | Diagnose coverage lookup for an uninformed persuasion seller: exact/coarse support keys consume `v/c`, and coarse construction coerces missing values into zero bins. | Mechanism confirmed statically; activation/effect in the 50-game batch is unknown because observations do not record `GLEE_SUPPORT_INDEX` or internal control payloads. No policy change authorized by this audit. | Reachable chain: `_persuasion` -> `_control` -> `_context_coverage` -> `context_support_lookup` -> `_coarse_config(v,c)` -> uncertainty/mode -> late low-quality recommendation. | `candidate` | unassigned | Shared coverage/control surface; must preserve the seller's information boundary rather than inject hidden true values. | `my_agents/jordan_strategic.py`; `glee_eval/data/dataset_audit.py`; `docs/FAILED.md` |
...
| `live_terminal_results` | Audit reconstructible outcomes with `live-episodes` and prospectively capture full SDK move responses. | Existing observation log has no authoritative terminal result; opponent-ended games remain absent even from prospective move-boundary capture. | 109 games: bargaining 15 reconstructed/21 indeterminate; negotiation 13/23 (7 lack normalization); persuasion 19/18. Comparable family means and shadow ratings are unavailable. | `candidate` | unassigned | Requires read-only authoritative server result backfill/export before historical batch comparison; no live game is authorized. | `docs/LIVE_INTEGRATION.md`; `glee_eval/live/episodes.py`; `glee_eval/live/run.py` |
